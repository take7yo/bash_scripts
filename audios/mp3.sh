...
curl ${url} > ${filename}
# 抓取链接节点
echo "cat ${filename} | grep -i '.*thumb=\"\(.*.mp3\)\">' > ${links}"
cat ${filename} | grep -i '.*thumb=\"\(.*.mp3\)\">' > ${links}
# 抓取文件名节点
echo "cat ${filename} | grep '<a target=\"_blank\" href=\".*\" alt=\"\(.*\)\">.*</a>' > ${names}"
cat ${filename} | grep '<a target=\"_blank\" href=\".*\" alt=\"\(.*\)\">.*</a>' > ${names}
# 抓取链接列表
echo 'sed "s/.*thumb=\"\(.*.[mM][pP]3\)\">/\1/g"' ${links}
sed "s/.*thumb=\"\(.*.[mM][pP]3\)\">/\1/g" ${links} > ${filename}
cat ${filename} > ${links}
# echo "cat ${links}"
# cat ${links}
# 统计音频个数
...
for (( i=1; i<=${filenum}; i++ ))
	...
	oldfn=`sed "s/.*\/\(.*.[mM][pP]3\)/\1/g" ${linkfile} | tr -d '\r'`
	newfn=`sed -n ${i}p ${names} | tr -d '\r'`
	echo "old file name is ${oldfn}"
	echo "new file name is ${newfn}.mp3"
	mv "${oldfn}" "${newfn}.mp3"
done
# 删除临时文件
rm -f *.txt